...
static bool loadEnemyActions(EnemyConfig* enemyConfig, char* configFilePath)
{
    ...
    {
        TraceLog(LOG_ERROR, "Echec d'initialisation du parser pour le chargement des actions ennemie");
        return false;
    }
    ...
    while (countOfElementsToGet != 0 && !stop)
    {
        ...
        {
            TraceLog(LOG_ERROR, "Echec de récupération du token lors du chargement des actions ennemie");
            return false;
        }
            ...
            case YAML_SCALAR_TOKEN:
                switch (keyCount)
                {
                    case 1:
                        ...
                        strncpy(enemy.id, (char*)token->data.scalar.value,
                                sizeof(char) * (SUPPOSED_ENEMY_ID_MAX_LEN - 1));
                        break;
                    ...
                    case 6:
                        ...
                        strncpy(enemy.name, (char*)token->data.scalar.value,
                                sizeof(char) * (SUPPOSED_ENEMY_NAME_MAX_LEN - 1));
                        yaml_token_delete(token);
...
void* loadEnemies(yaml_parser_t* parser, const char* parentDirPath)
{
    ...
    if (config == NULL)
    {
        TraceLog(LOG_ERROR, "Echec de l'allocation de la configuration ennemie");
        return NULL;
    }
    ...
    {
        free(config);
        TraceLog(LOG_ERROR, "Echec d'allocation le map des ennemies");
        return NULL;
    }
    ...
    while (true)
    {
        ...
        {
            TraceLog(LOG_ERROR, "\nEchec de récupération de la configuration ennemie");
            break;
        }
