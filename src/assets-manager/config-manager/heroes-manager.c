    ...
    while (countOfElementsToGet != 0 && !stop)
    {
        ...
        switch (token.type)
        {
            ...
            case YAML_SCALAR_TOKEN:
                ...
                memset(heroConfig->actionsConfigs[actionKey].actionName, 0,
                       sizeof(char) * SUPPOSED_HERO_ACTION_NAME_MAX_LEN);
                strncpy(
            ...
            case YAML_SCALAR_TOKEN:
                switch (keyCount)
                {
                    case 1:
                        ...
                        strncpy(hero.id, (char*)token->data.scalar.value,
                                sizeof(char) * (SUPPOSED_HERO_ID_MAX_LEN - 1));
                        break;
                    ...
                    case 6:
                        ...
                        strncpy(hero.name, (char*)token->data.scalar.value,
                                sizeof(char) * (SUPPOSED_HERO_NAME_MAX_LEN - 1));
                        yaml_token_delete(token);
